...
/*
	...
	UPD 16.11.20: После семинара добавляю ID книг в каталог
	UPD 29.11.20: Работаем с OMP
...
public:
	...
	void setBookToCatalog(const int bookId, const coordBook bookPlace) {
		...
		mapMtx.lock();
		catalog.insert(std::make_pair(bookId, bookPlace));
		mapMtx.unlock();
	}
...
public:
	...
	void addBook(const int& row, const int& bookshelf, const int& bookPlace, const int& bookID, Catalog& catalog);
...
#pragma omp parallel for
	for (int i = 0; i < bookshelfsRows; i++)
		for (int j = 0; j < bookshelfs; j++)
			for (int k = 0; k < maxBooks; k++)
				library[i][j].addBook(i, j, k, bookID++, catalog);
}
...
void Bookshelf::addBook(const int& row, const int& bookshelf, const int& bookPlace, const int& bookID, Catalog& catalog) {
